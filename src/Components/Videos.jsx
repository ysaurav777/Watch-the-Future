  ...
  return ( 
    <Stack direction={['column','row']} h={'100vh'}>
        <VStack w={'full'}>
            ...
            <VStack alignItems={'flex-start'} p={'8'} w={'fill'}>
                <Heading>Sample Video 1</Heading>
                <Text>This is a testing phase please corporate.</Text>
            </VStack>
        </VStack>
        <VStack w={['full','xl']} alignItems={'stretch'} p={'8'} spacing={'8'} overflowY={'auto'}>
            {
                videosArr.map((items,index)=> (
                    <Button variant={'ghost'} colorScheme='green' onClick={()=>setvideosrc(items)}>
                        Videos {index+1}
                    </Button>
                ))
            }
